...
def load_and_preprocess_all_episodes(demo_paths, out_dataset_dir):
    ...
    for demo in tqdm(demo_paths):
        qpos, qvel, effort, action, image_dict, is_sim = load_hdf5(f"{demo}/main.rmb.hdf5")
        # Save non-image info
...
def main(args):
    ...
    all_demo_paths = glob.glob(os.path.join(args.dataset_path, "*.rmb"))  # List of HDF5 filepaths
    all_demo_paths.sort()
    ...
    for train in train_demo_paths:
        shutil.copytree(train, os.path.join(out_dataset_dir_train, os.path.basename(train)), dirs_exist_ok=True)
    for val in val_demo_paths:
        shutil.copytree(val, os.path.join(out_dataset_dir_val, os.path.basename(val)), dirs_exist_ok=True)
